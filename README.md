...
## 📱 安裝至手機/電腦主畫面 (PWA 安裝指南)

本專案支援 **PWA (Progressive Web App)** 技術，您可將其安裝至手機或電腦桌面，以獲得類似原生 App 的獨立全螢幕體驗（不顯示瀏覽器網址列）：

### 🤖 Android (Chrome / Edge)
1. 用 Chrome 瀏覽器開啟您的 HTTPS 部署網址（或本機 `http://localhost:8000`）。
2. 點選瀏覽器右上角 **三點選單**。
3. 點擊 **「加入主畫面」**（或 **「安裝應用程式」**）。
4. 點選「新增」後，手機主畫面便會出現「聽新聞」圖示，一鍵即可直接開啟。

### 🍏 iOS (Safari)
1. 用 Safari 瀏覽器開啟您的 HTTPS 部署網址。
2. 點選底部工具列的 **分享** 圖示（向上箭頭與方框組合）。
3. 往下滑動，選取 **「加入主畫面」**。
4. 點擊右上角「新增」即可完成安裝。

### 💻 電腦版 (Chrome / Edge / Opera)
1. 開啟部署網址後，在瀏覽器最上方的 **網址列最右端** 尋找 **「電腦配上向下箭頭」** 或 **`⊕` (安裝)** 圖示。
2. 點選該圖示並按「安裝」即可在桌面建立捷徑。
3. 亦可點選瀏覽器右上角 **三點選單 →「投放、儲存及分享」→「安裝頁面為應用程式...」** 進行安裝。
